...
<div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
        <label for="global_location_id" class="text-[10px] font-black uppercase text-gray-400 tracking-wider whitespace-nowrap">
            {{ __('Location:') }}
        </label>
        @if ($locations->isEmpty())
            <span class="text-xs font-semibold text-gray-400 italic">
        ...
        @else
            <select 
                ...
                class="px-3.5 py-1.5 text-xs font-bold bg-gray-50 text-indigo-600 border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition duration-150 cursor-pointer"
            >
    ...
    <!-- Turf Selector -->
    <div class="flex items-center gap-2 w-full sm:w-auto">
        <label for="global_turf_id" class="text-[10px] font-black uppercase text-gray-400 tracking-wider whitespace-nowrap">
            {{ __('Turf:') }}
        </label>
        @if ($locations->isEmpty())
            <span class="text-xs font-semibold text-gray-400 italic">
