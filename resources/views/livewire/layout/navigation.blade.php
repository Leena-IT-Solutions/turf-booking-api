            ...
            <nav class="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
                ...
                @if (auth()->user()->hasRole('turf-admin'))
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('turf.support') }}" wire:navigate class="flex items-center justify-between px-4 py-3 text-sm font-medium rounded-xl transition duration-150 ease-in-out {{ request()->routeIs('turf.support') ? 'bg-indigo-50/70 text-indigo-600 dark:bg-indigo-950/20 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-100' }}">
                            <div class="flex items-center">
                                <svg class="h-5 w-5 text-gray-500 {{ request()->routeIs('turf.support') ? 'text-indigo-600 dark:text-indigo-400' : '' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <nav class="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
                ...
                @if (auth()->user()->hasRole('turf-admin'))
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('turf.support') }}" wire:navigate @click="sidebarOpen = false" class="flex items-center justify-between px-4 py-3 text-sm font-medium rounded-xl transition duration-150 ease-in-out {{ request()->routeIs('turf.support') ? 'bg-indigo-50/70 text-indigo-600 dark:bg-indigo-950/20 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-100' }}">
                            <div class="flex items-center">
                                <svg class="h-5 w-5 text-gray-500 {{ request()->routeIs('turf.support') ? 'text-indigo-600 dark:text-indigo-400' : '' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                </svg>
