...
new #[Layout('layouts.app')] class extends Component
{
    ...
    {
        $this->reset(['equipmentId', 'name', 'icon', 'is_active', 'generatedIconOptions']);
        $this->resetErrorBag();
...
<div class="py-6" x-data="{ formModal: @entangle('showFormModal'), deleteConfirm: @entangle('showDeleteConfirm') }">
    <div class="sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-xl font-bold text-gray-900">{{ __('Global Equipments Repository') }}</h2>
                <p class="text-xs text-gray-400 mt-1">{{ __('Manage master equipment and rental gear inventory available for select configurations at the individual turf field level.') }}</p>
            </div>
        ...
        @if ($equipments->isEmpty())
            ...
            <div class="bg-white p-16 rounded-3xl border border-gray-100 shadow-sm text-center">
                ...
                <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wider">{{ __('No Equipments Options Found') }}</h3>
                <p class="text-xs text-gray-400 mt-2 max-w-xs mx-auto leading-relaxed">{{ __('Add equipment options to populate the global selector repository.') }}</p>
            </div>
        @else
                    ...
                    <div class="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition duration-200">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="inline-flex items-center gap-1">
                                <button type="button" wire:click="openEditModal({{ $equipmentItem->id }})" class="p-1.5 text-gray-400 hover:text-indigo-600 transition cursor-pointer" title="{{ __('Edit') }}">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button type="button" wire:click="confirmDelete({{ $equipmentItem->id }})" class="p-1.5 text-gray-400 hover:text-red-600 transition cursor-pointer" title="{{ __('Delete') }}">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
                            ...
                            <button type="button" wire:click="toggleActive({{ $equipmentItem->id }})" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[10px] font-bold cursor-pointer transition {{ $equipmentItem->is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-100/50 ' : 'bg-gray-50 text-gray-500 border border-gray-200 ' }}">
                    ...
                    <div class="p-6">
                        ...
                        <form wire:submit="saveEquipment" class="space-y-4 mt-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="name" wire:model="name" class="block w-full px-4 py-3 text-sm rounded-xl bg-gray-50 border border-gray-250 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition duration-150 ease-in-out" placeholder="e.g. FIFA Pro Soccer Balls, Tennis Rackets">
                            ...
                            <div class="flex items-center justify-between p-3 rounded-2xl bg-gray-55 border border-gray-250">
                                <div class="flex flex-col">
                                    <span class="text-xs font-bold text-gray-850">{{ __('Equipment Active Status') }}</span>
                ...
                <div x-show="deleteConfirm" class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-100">
                    <div class="p-6">
                        ...
                        <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                            <button type="button" @click="deleteConfirm = false" class="px-4 py-2 text-xs font-bold text-gray-700 border border-gray-250 rounded-xl hover:bg-gray-55 transition cursor-pointer">
                                {{ __('Cancel') }}
                            </button>
