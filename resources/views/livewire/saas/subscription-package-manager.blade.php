...
<div class="py-6 w-full">
    <div class="w-full px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
            <div>
                <h1 class="text-2xl font-black text-gray-900 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-600 to-violet-500 text-white flex items-center justify-center shadow-lg shadow-indigo-500/30 shrink-0">
        ...
        <!-- Flash Messages -->
        @if (session('status'))
            <div class="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center gap-2.5">
        ...
        <!-- Filter & Search Bar -->
        <div class="bg-white rounded-3xl p-4 sm:p-5 shadow-xs border border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4 w-full">
            ...
            <div class="relative w-full md:w-96">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search package name or details..." 
                    class="w-full pl-10 pr-4 py-2.5 text-xs rounded-xl border border-gray-200 bg-white text-gray-900 focus:border-indigo-500 focus:ring-indigo-500">
            </div>

            <!-- Filter Controls -->
            <div class="flex items-center gap-3 w-full md:w-auto">
                <select wire:model.live="statusFilter" class="py-2.5 px-3 text-xs rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="all">All Packages</option>
                    <option value="active">Active Only</option>
                    <option value="inactive">Inactive Only</option>
                </select>

                <select wire:model.live="sortBy" class="py-2.5 px-3 text-xs rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-indigo-500">
        ...
        <div class="flex flex-col space-y-4 w-full">
            @forelse ($packages as $pkg)
                <div class="bg-white rounded-3xl border border-gray-200 p-6 shadow-sm flex flex-col space-y-5 transition hover:border-indigo-400 hover:shadow-md w-full">
                    ...
                    <div class="flex items-start justify-between gap-4 w-full">
                        <div class="flex items-center gap-3">
                            ...
                            <button wire:click="toggleActive({{ $pkg->id }})" type="button" 
                                ...
                                class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase transition cursor-pointer shrink-0 {{ $pkg->is_active ? 'bg-emerald-100 text-emerald-800 border border-emerald-300 ' : 'bg-gray-100 text-gray-600 border border-gray-300 ' }}">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <button wire:click="confirmDelete({{ $pkg->id }})" type="button" 
                                class="p-2 bg-red-600/10 hover:bg-red-600/20 text-red-600 border border-red-500/30 rounded-xl text-xs font-bold transition flex items-center justify-center cursor-pointer">
                    ...
                    <div class="w-full bg-gray-50/90 p-4 rounded-2xl border border-gray-100 flex flex-wrap sm:flex-nowrap items-center justify-between gap-4">
                        <div class="flex items-center gap-6 flex-wrap sm:flex-nowrap">
                            <div>
                                ...
                                <span class="text-xl font-black text-gray-900">₹{{ number_format($pkg->monthly_amount, 2) }}</span>
                            </div>
                            <div class="hidden sm:block h-8 w-px bg-gray-200"></div>
                            <div>
                                <span class="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider block">YEARLY (365 DAYS)</span>
                                <span class="text-xl font-black text-indigo-600">₹{{ number_format($pkg->yearly_amount, 2) }}</span>
                            </div>
                        </div>

                        <div class="px-4 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-center shrink-0">
                            <span class="text-[10px] font-black text-amber-600 uppercase tracking-wider block">COMMISSION</span>
                            <span class="text-base font-black text-amber-600">{{ $pkg->commission_percentage }}%</span>
                        </div>
                    </div>

                </div>
            @empty
                <div class="w-full bg-white p-12 rounded-3xl border border-gray-200 text-center text-gray-500 space-y-3">
                    <span class="text-4xl block">📦</span>
                    <p class="font-bold text-gray-800">No subscription packages found.</p>
    ...
    @if ($showFormModal)
        <div class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/70 backdrop-blur-xs flex items-center justify-center p-4">
            <div class="w-full max-w-2xl bg-white rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 border border-gray-200">
                <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                    <div>
                        <span class="text-[10px] font-black uppercase tracking-wider text-indigo-600">SAAS Package Configuration</span>
                        <h2 class="text-xl font-black text-gray-900 mt-0.5">
                            {{ $editingId ? 'Edit Subscription Package' : 'Create New Subscription Package' }}
                        </h2>
                    </div>
                    <button wire:click="$set('showFormModal', false)" class="p-2 text-gray-400 hover:text-gray-600 rounded-xl hover:bg-gray-100">✕</button>
                </div>
                ...
                <form wire:submit.prevent="savePackage" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="name" placeholder="e.g. Pro Subscription" 
                            class="w-full p-2.5 text-xs rounded-xl border border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Monthly Amount (₹) *</label>
                            <input type="number" step="0.01" wire:model="monthly_amount" placeholder="499.00" 
                                class="w-full p-2.5 text-xs rounded-xl border border-gray-300 bg-white text-gray-900 font-bold">
                            @error('monthly_amount') <span class="text-[10px] text-red-500 font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Yearly Amount (₹) *</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Display Sort Order</label>
                            <input type="number" wire:model="sort_order" placeholder="0" 
                                class="w-full p-2.5 text-xs rounded-xl border border-gray-300 bg-white text-gray-900">
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Commission Percentage (%) *</label>
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" rows="2" placeholder="Brief package description..." 
                            class="w-full p-2.5 text-xs rounded-xl border border-gray-300 bg-white text-gray-900"></textarea>
                    </div>

                    <!-- Features (One per line) -->
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Features (One per line)</label>
                        <textarea wire:model="features_text" rows="3" placeholder="Single Turf Management&#10;Custom Coupon & Slot Locks&#10;Priority 24/7 Phone Support" 
                            class="w-full p-2.5 text-xs rounded-xl border border-gray-300 bg-white text-gray-900 font-mono"></textarea>
                    </div>
                    ...
                    <div class="flex items-center gap-2 pt-2">
                        ...
                        <label for="is_active_check" class="text-xs font-bold text-gray-800 cursor-pointer">
                            Is Active
                        </label>
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-100 flex justify-end gap-2">
                        <button type="button" wire:click="$set('showFormModal', false)" class="px-4 py-2.5 bg-gray-100 text-gray-700 rounded-xl text-xs font-bold">Cancel</button>
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/70 backdrop-blur-xs flex items-center justify-center p-4">
            <div class="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl space-y-4 border border-gray-200">
                <div class="flex items-center gap-3 text-red-600">
                    ...
                    <div>
                        <h3 class="font-black text-lg text-gray-900">Delete Package</h3>
                        <p class="text-xs text-gray-500">Are you sure you want to delete this subscription package?</p>
                    </div>
                </div>

                <div class="pt-3 border-t border-gray-100 flex justify-end gap-2">
                    <button wire:click="$set('showDeleteModal', false)" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-xl text-xs font-bold">Cancel</button>
