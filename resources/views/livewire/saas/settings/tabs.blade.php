...
<div class="bg-white p-2 rounded-2xl border border-gray-100 shadow-xs">
    <nav class="flex flex-wrap items-center gap-1.5" aria-label="Settings Tabs">
        ...
        <!-- Notifications Tab -->
        <a href="{{ route('saas.settings.notifications') }}" wire:navigate 
           class="flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-xl transition duration-150 ease-in-out {{ $active === 'notifications' ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-500/20' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' }}">
            <svg class="w-4 h-4 {{ $active === 'notifications' ? 'text-white' : 'text-gray-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
