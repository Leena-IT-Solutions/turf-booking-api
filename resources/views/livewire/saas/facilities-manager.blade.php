...
new #[Layout('layouts.app')] class extends Component
{
    ...
    {
        $this->reset(['facilityId', 'name', 'icon', 'is_active', 'generatedIconOptions']);
        $this->resetErrorBag();
    ...
    {
        if ($this->deletingId) {
            $facility = Facility::findOrFail($this->deletingId);
            $facility->delete();
...
<div class="py-6" x-data="{ formModal: @entangle('showFormModal'), deleteConfirm: @entangle('showDeleteConfirm') }">
    <div class="sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-xl font-bold text-gray-900">{{ __('Global Facilities Repository') }}</h2>
                <p class="text-xs text-gray-400 mt-1">{{ __('Manage master facility names available for select configurations at the individual turf field level.') }}</p>
            </div>
        ...
        <div class="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-4">
            <div class="flex-grow max-w-md relative">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" class="block w-full pl-10 pr-4 py-2.5 text-xs rounded-xl bg-gray-50 border border-gray-250 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition duration-150 ease-in-out" placeholder="{{ __('Search global facility names...') }}">
            </div>
        </div>
        ...
        @if ($facilities->isEmpty())
            ...
            <div class="bg-white p-16 rounded-3xl border border-gray-100 shadow-sm text-center">
                <div class="h-16 w-16 rounded-2xl bg-indigo-50/50 text-indigo-500 flex items-center justify-center mx-auto mb-4 border border-indigo-100/30">
                    <svg class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                </div>
                <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wider">{{ __('No Facilities Options Found') }}</h3>
                <p class="text-xs text-gray-400 mt-2 max-w-xs mx-auto leading-relaxed">{{ __('Add facility options to populate the global selector repository.') }}</p>
            </div>
        @else
            <!-- Grid list of Facility Options (Card Design) -->
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($facilities as $facilityItem)
                    <div class="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition duration-200">
                        <div class="flex items-start justify-between gap-4">
                            <!-- Icon display -->
                            <div class="h-12 w-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100/50 shrink-0">
                                <x-icon :name="$facilityItem->icon" class="h-6 w-6" />
                            </div>
                            <!-- Card Actions -->
                            <div class="inline-flex items-center gap-1">
                                <button type="button" wire:click="openEditModal({{ $facilityItem->id }})" class="p-1.5 text-gray-400 hover:text-indigo-600 transition cursor-pointer" title="{{ __('Edit') }}">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button type="button" wire:click="confirmDelete({{ $facilityItem->id }})" class="p-1.5 text-gray-400 hover:text-red-600 transition cursor-pointer" title="{{ __('Delete') }}">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
                            ...
                            <button type="button" wire:click="toggleActive({{ $facilityItem->id }})" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[10px] font-bold cursor-pointer transition {{ $facilityItem->is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-100/50 ' : 'bg-gray-50 text-gray-500 border border-gray-200 ' }}">
            ...
            @if ($facilities->hasPages())
                <div class="mt-6 bg-white px-6 py-4 rounded-3xl border border-gray-100 shadow-sm">
                    {{ $facilities->links() }}
                </div>
            @endif
        @endif
        ...
        <div x-show="formModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div x-show="formModal" @click="formModal = false" class="fixed inset-0 transition-opacity bg-gray-950/40 backdrop-blur-sm"></div>

                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div x-show="formModal" class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-100">
                    <div class="p-6">
                        <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                            <h3 class="text-base font-bold text-gray-900">
                                {{ $isEditing ? __('Edit Global Facility Option') : __('Add Global Facility Option') }}
                            </h3>
                            <button type="button" @click="formModal = false" class="text-gray-400 hover:text-gray-500 cursor-pointer">
                        ...
                        <form wire:submit="saveFacility" class="space-y-4 mt-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="name" wire:model="name" class="block w-full px-4 py-3 text-sm rounded-xl bg-gray-50 border border-gray-250 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition duration-150 ease-in-out" placeholder="e.g. Locker Room, Free Parking">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-2">
                                    <label for="icon" class="block text-xs font-bold text-gray-700 uppercase tracking-wider">
                                ...
                                <div class="flex gap-2">
                                    <div class="relative flex-grow">
                                        <input type="text" id="icon" wire:model="icon" class="block w-full px-4 py-3 text-xs font-mono rounded-xl bg-gray-55 border border-gray-250 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition duration-150 ease-in-out" placeholder="e.g. wifi, or paste raw <svg>...</svg>">
                                    </div>
                                    @if ($icon)
                                        <div class="h-[46px] w-[46px] bg-gray-55 border border-gray-250 rounded-2xl flex items-center justify-center shrink-0">
                                ...
                                <p class="text-[10px] text-gray-400 mt-2 leading-relaxed">
                                    Supported pre-compiled names: <code class="bg-gray-55 px-1 py-0.5 rounded text-indigo-500 font-mono text-[9px]">wifi, parking, shower, water, light, first-aid, coffee, seating, key, football, cricket, tennis, basketball, sun, sunset, moon</code>. Find emojis at <a href="https://emojipedia.org" target="_blank" class="text-indigo-600 hover:underline">Emojipedia</a>, or click the AI button to generate a custom SVG vector instantly using Gemini.
                                </p>
                            </div>

                            <!-- Is Active status toggle -->
                            <div class="flex items-center justify-between p-3 rounded-2xl bg-gray-55 border border-gray-250">
                                <div class="flex flex-col">
                                    <span class="text-xs font-bold text-gray-850">{{ __('Facility Active Status') }}</span>
                                ...
                                <button type="button" @click="@this.set('is_active', !@this.get('is_active'))" class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none" :class="@this.get('is_active') ? 'bg-indigo-600' : 'bg-gray-200 '">
                            ...
                            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                                <button type="button" @click="formModal = false" class="px-4 py-2 text-xs font-bold text-gray-700 border border-gray-250 rounded-xl hover:bg-gray-50 transition cursor-pointer">
                                    {{ __('Cancel') }}
                                </button>
        ...
        <div x-show="deleteConfirm" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="deleteConfirm" class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-100">
                    <div class="p-6">
                        <div class="flex items-center gap-4">
                            <div class="h-10 w-10 shrink-0 rounded-xl bg-red-50 text-red-500 flex items-center justify-center border border-red-100/50">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-gray-900">
                                    {{ __('Delete Facility Option') }}
                                </h3>
                                <p class="text-xs text-gray-400 mt-1">
                                    {{ __('Are you sure you want to permanently delete this master facility option? This will delete all linkage and cannot be undone.') }}
                                </p>
                            </div>
                        </div>

                        <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                            <button type="button" @click="deleteConfirm = false" class="px-4 py-2 text-xs font-bold text-gray-700 border border-gray-250 rounded-xl hover:bg-gray-50 transition cursor-pointer">
                                {{ __('Cancel') }}
                            </button>
