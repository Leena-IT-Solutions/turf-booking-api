...
new #[Layout('layouts.app')] class extends Component
{
    ...
    {
        if ($this->deletingId) {
            $this->deleteSlide($this->deletingId);
        }
        $this->cancelDelete();
    }
                    ...
                    <div class="relative aspect-[16/9] bg-gray-900 rounded-3xl overflow-hidden shadow-sm group border border-gray-100 dark:border-gray-800 cursor-move transition-all duration-300"
                        ...
                        <div class="absolute top-4 inset-x-4 flex justify-between items-start z-10">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button wire:click="confirmDelete({{ $sliderImage->id }})" class="p-2 bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-md transition transform hover:scale-105 cursor-pointer flex items-center justify-center" style="background-color: rgb(220, 38, 38);">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
