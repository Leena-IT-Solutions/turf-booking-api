                            ...
                            @foreach ($allFacilities as $facilityItem)
                                <label class="relative flex items-start p-4 rounded-2xl border transition duration-150 cursor-pointer {{ in_array((string)$facilityItem->id, $selectedFacilityIds) ? 'bg-indigo-50/20 border-indigo-250 dark:bg-indigo-950/10 dark:border-indigo-900/50' : 'bg-transparent border-gray-100 hover:bg-gray-50/50 dark:border-gray-700 dark:hover:bg-gray-800/30' }}">
                                    ...
                                    <div class="ms-3 text-sm">
                                        <span class="font-semibold text-gray-900 dark:text-gray-100">{{ $facilityItem->name }}</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>
