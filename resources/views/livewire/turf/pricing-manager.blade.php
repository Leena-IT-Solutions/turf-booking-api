    ...
    <div class="sm:px-6 lg:px-8 space-y-6">
        ...
        @if (!$turf)
            ...
            <div class="bg-white p-16 rounded-3xl border border-gray-100 shadow-sm text-center">
                ...
                <p class="text-xs text-gray-400 mt-2 max-w-sm mx-auto leading-relaxed">{{ __('Please add a Location and Turf first, or choose one from the selector in the top bar to configure pricing.') }}</p>
            </div>
        @else
            <!-- Header Section -->
            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">{{ __('Pricing for') }} <span class="text-indigo-600">{{ $turf->name }}</span></h2>
                    <p class="text-xs text-gray-400 mt-1">{{ __('Manage your court booking prices, surcharges, and packages here.') }}</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    <div>
                        <div class="flex items-center justify-between pb-4 border-b border-gray-50 gap-4">
                            <div class="flex items-center gap-2">
                                <span class="flex items-center justify-center h-8 w-8 rounded-lg bg-indigo-50 text-indigo-600 shrink-0">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 9.172V5L8 4z" />
                                    </svg>
                                </span>
                                <div>
                                    <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider">{{ __('Pricing Wizard') }}</h3>
                                    <p class="text-[10px] text-gray-400 font-semibold mt-0.5">{{ __('Step') }} {{ $step }} {{ __('of 2') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        @if ($step === 1)
                            ...
                            <div class="space-y-4 py-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <!-- Yes Option -->
                                    <label class="relative p-5 rounded-2xl border transition duration-150 cursor-pointer flex flex-col justify-between h-32 {{ $sameRateThroughoutWeek === 'yes' ? 'bg-indigo-50/40 border-indigo-600 ring-2 ring-indigo-600/10' : 'bg-transparent border-gray-200 hover:bg-gray-50/50 ' }}">
                                        <div class="flex items-center justify-between">
                                            <span class="font-bold text-sm text-gray-900">{{ __('Yes, same price everyday') }}</span>
                                            <input type="radio" wire:model.live="sameRateThroughoutWeek" value="yes" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300">
                                        </div>
                                    ...
                                    <!-- No Option -->
                                    <label class="relative p-5 rounded-2xl border transition duration-150 cursor-pointer flex flex-col justify-between h-32 {{ $sameRateThroughoutWeek === 'no' ? 'bg-indigo-50/40 border-indigo-600 ring-2 ring-indigo-600/10' : 'bg-transparent border-gray-200 hover:bg-gray-50/50 ' }}">
                                        <div class="flex items-center justify-between">
                                            <span class="font-bold text-sm text-gray-900">{{ __('No, rates are different') }}</span>
                                            <input type="radio" wire:model.live="sameRateThroughoutWeek" value="no" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300">
                                        </div>
                                        <p class="text-[10px] text-gray-400 leading-relaxed mt-2">
                            ...
                            <div class="space-y-6 py-4">
                                @if ($sameRateThroughoutWeek === 'yes')
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <!-- Flat Rate Option -->
                                            <label class="relative p-4 rounded-xl border transition duration-150 cursor-pointer flex items-center justify-between {{ $sameRateThroughoutDayAll === 'yes' ? 'bg-indigo-50/30 border-indigo-600 ring-2 ring-indigo-600/10' : 'bg-transparent border-gray-200 hover:bg-gray-50/50 ' }}">
                                                <span class="font-bold text-xs text-gray-900">{{ __('Yes, flat rate all day') }}</span>
                                            ...
                                            <label class="relative p-4 rounded-xl border transition duration-150 cursor-pointer flex items-center justify-between {{ $sameRateThroughoutDayAll === 'no' ? 'bg-indigo-50/30 border-indigo-600 ring-2 ring-indigo-600/10' : 'bg-transparent border-gray-200 hover:bg-gray-50/50 ' }}">
                                                <span class="font-bold text-xs text-gray-900">{{ __('No, dynamic rates') }}</span>
                                        ...
                                        @if ($sameRateThroughoutDayAll === 'yes')
                                            <!-- Flat Price Input -->
                                            <div class="p-4 bg-gray-50/40 border border-gray-100 rounded-2xl space-y-2">
                                                <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{{ __('Flat Rate per Hour (₹)') }}</label>
                                                <div class="relative rounded-xl shadow-sm max-w-xs">
                                                    <div class="absolute inset-y-0 start-0 ps-3 flex items-center pointer-events-none">
                                                        <span class="text-gray-500 text-xs font-semibold">₹</span>
                                                    </div>
                                                    <input type="number" wire:model="flatRateAll" required class="block w-full ps-7 pe-3 py-2 text-xs font-mono font-bold rounded-xl border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600 transition" placeholder="500">
                                                </div>
                                            </div>
                                        @else
                                            <!-- Dynamic Time-Range Repeaters -->
                                            <div class="p-5 bg-gray-50/40 border border-gray-100 rounded-2xl space-y-4">
                                                <div class="flex items-center justify-between border-b border-gray-100 pb-2">
                                                    <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{{ __('Time Slices & Surcharges') }}</span>
                                                    <button type="button" wire:click="addTimeRange('all')" class="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 transition cursor-pointer">
                                                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                                ...
                                                <div class="space-y-3">
                                                    @foreach ($timeRangesAll as $idx => $range)
                                                        <div wire:key="time-range-all-{{ $idx }}" class="flex flex-col sm:flex-row items-center gap-3 bg-white p-3 rounded-xl border border-gray-200/40 shadow-sm relative">
                                                            <!-- From Time -->
                                                            <div class="w-full">
                                                                <label class="block text-[8px] font-bold text-gray-400 uppercase mb-1">{{ __('From') }}</label>
                                                                <input type="time" wire:model="timeRangesAll.{{ $idx }}.from" required class="block w-full py-1.5 px-2 text-xs font-mono font-semibold rounded-lg bg-white border border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600">
                                                            </div>
                                                            <!-- To Time -->
                                                            <div class="w-full">
                                                                <label class="block text-[8px] font-bold text-gray-400 uppercase mb-1">{{ __('To') }}</label>
                                                                <input type="time" wire:model="timeRangesAll.{{ $idx }}.to" required class="block w-full py-1.5 px-2 text-xs font-mono font-semibold rounded-lg bg-white border border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600">
                                                            </div>
                                                            <!-- Rate -->
                                                            <div class="w-full">
                                                                <label class="block text-[8px] font-bold text-gray-400 uppercase mb-1">{{ __('Rate/Hr (₹)') }}</label>
                                                                <input type="number" wire:model="timeRangesAll.{{ $idx }}.rate" required class="block w-full py-1.5 px-2 text-xs font-mono font-bold rounded-lg bg-white border border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600" placeholder="600">
                                                            </div>
                                                            <!-- Delete Button -->
                                ...
                                @else
                                    ...
                                    <div class="space-y-6">
                                        <div class="flex items-center justify-between pb-2 border-b border-gray-100">
                                            ...
                                            <button type="button" wire:click="addDayGroup" class="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 transition cursor-pointer bg-indigo-50 px-3 py-1.5 rounded-xl border border-indigo-100/30">
                                                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                                                </svg>
                                                {{ __('Add Day Group') }}
                                         ...
                                         <div class="space-y-6 pb-2">
                                             @foreach ($dayGroups as $gIndex => $group)
                                                 <div wire:key="day-group-{{ $gIndex }}" class="p-5 bg-gray-50/40 border border-gray-200/60 rounded-3xl space-y-4 relative">
                                                     <!-- Remove Group button -->
                                                     ...
                                                     <div class="space-y-2">
                                                         <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{{ __('1. Select days in this group:') }}</span>
                                                         
                                                         <div class="flex flex-wrap gap-2">
                                                             @foreach (['mon' => 'Mon', 'tue' => 'Tue', 'wed' => 'Wed', 'thu' => 'Thu', 'fri' => 'Fri', 'sat' => 'Sat', 'sun' => 'Sun'] as $dayVal => $dayLbl)
                                                                 <label wire:key="day-checkbox-{{ $gIndex }}-{{ $dayVal }}" class="relative flex items-center px-3.5 py-1.5 rounded-xl border text-[10px] font-bold transition duration-100 cursor-pointer select-none {{ in_array($dayVal, $group['days'] ?? []) ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm' : 'bg-transparent text-gray-500 hover:bg-gray-50/50 border-gray-300 ' }}">
                                                     ...
                                                     <div class="space-y-3 pt-2">
                                                         <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider block">{{ __('2. Rates configuration:') }}</span>
                                                         <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                             <label class="relative p-3.5 rounded-xl border transition duration-150 cursor-pointer flex items-center justify-between {{ $group['sameRateThroughoutDay'] === 'yes' ? 'bg-indigo-50/20 border-indigo-600 ring-2 ring-indigo-600/10' : 'bg-transparent border-gray-200 hover:bg-gray-50/50 ' }}">
                                                                 <span class="font-bold text-xs text-gray-900">{{ __('Flat rate all day') }}</span>
                                                             ...
                                                             <label class="relative p-3.5 rounded-xl border transition duration-150 cursor-pointer flex items-center justify-between {{ $group['sameRateThroughoutDay'] === 'no' ? 'bg-indigo-50/20 border-indigo-600 ring-2 ring-indigo-600/10' : 'bg-transparent border-gray-200 hover:bg-gray-50/50 ' }}">
                                                                 <span class="font-bold text-xs text-gray-900">{{ __('Different time range rates') }}</span>
                                                     ...
                                                     @if ($group['sameRateThroughoutDay'] === 'yes')
                                                         <div class="ps-1 max-w-xs space-y-1.5">
                                                             <label class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">{{ __('Flat Rate/Hr (₹)') }}</label>
                                                             <div class="relative rounded-xl shadow-sm">
                                                                 <div class="absolute inset-y-0 start-0 ps-3 flex items-center pointer-events-none">
                                                                     <span class="text-gray-500 text-xs font-semibold">₹</span>
                                                                 </div>
                                                                 <input type="number" wire:model="dayGroups.{{ $gIndex }}.flatRate" required class="block w-full ps-7 pe-3 py-2 text-xs font-mono font-bold rounded-xl border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600 transition" placeholder="600">
                                                             </div>
                                                         </div>
                                                     @else
                                                         ...
                                                         <div class="space-y-3 pt-2">
                                                             <div class="flex items-center justify-between pb-1 border-b border-gray-100">
                                                                 <span class="text-[9px] font-bold text-gray-400 uppercase">{{ __('Time Slices') }}</span>
                                                                 <button type="button" wire:click="addTimeRange('group', {{ $gIndex }})" class="text-[9px] font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 cursor-pointer">
                                                             ...
                                                             <div class="space-y-3">
                                                                 @foreach ($group['timeRanges'] as $tIdx => $range)
                                                                     <div wire:key="time-range-{{ $gIndex }}-{{ $tIdx }}" class="flex flex-col sm:flex-row items-center gap-3 bg-white p-3 rounded-xl border border-gray-200/40 shadow-sm relative">
                                                                         <div class="w-full">
                                                                             <label class="block text-[8px] font-bold text-gray-400 uppercase mb-1">{{ __('From') }}</label>
                                                                             <input type="time" wire:model="dayGroups.{{ $gIndex }}.timeRanges.{{ $tIdx }}.from" required class="block w-full py-1.5 px-2 text-xs font-mono font-semibold rounded-lg bg-white border border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600">
                                                                         </div>
                                                                         <div class="w-full">
                                                                             <label class="block text-[8px] font-bold text-gray-400 uppercase mb-1">{{ __('To') }}</label>
                                                                             <input type="time" wire:model="dayGroups.{{ $gIndex }}.timeRanges.{{ $tIdx }}.to" required class="block w-full py-1.5 px-2 text-xs font-mono font-semibold rounded-lg bg-white border border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600">
                                                                         </div>
                                                                         <div class="w-full">
                                                                             <label class="block text-[8px] font-bold text-gray-400 uppercase mb-1">{{ __('Rate/Hr (₹)') }}</label>
                                                                             <input type="number" wire:model="dayGroups.{{ $gIndex }}.timeRanges.{{ $tIdx }}.rate" required class="block w-full py-1.5 px-2 text-xs font-mono font-bold rounded-lg bg-white border border-gray-300 text-gray-900 focus:ring-indigo-500/20 focus:border-indigo-600" placeholder="600">
                                                                         </div>
                                ...
                                <div class="flex justify-between items-center pt-4 border-t border-gray-200">
                                    <button type="button" wire:click="prevStep" class="px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 text-xs font-bold rounded-xl transition cursor-pointer border border-gray-200 flex items-center gap-1.5">
                                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    <div class="flex items-center justify-between pb-4 border-b border-gray-50">
                            <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider">{{ __('Current Slot Rates') }}</h3>
                            <a href="{{ route('turf.slots') }}" wire:navigate class="text-xs text-indigo-600 hover:text-indigo-700 font-bold transition flex items-center gap-1">
                        ...
                        @if ($slots->isEmpty())
                            <div class="py-12 text-center text-xs text-gray-400">
                                <svg class="h-10 w-10 mx-auto mb-3 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                </svg>
                                    ...
                                    @if ($categorySlots->isNotEmpty())
                                        <div class="space-y-3">
                                            ...
                                            <div class="flex items-center gap-2 pb-1.5 border-b border-gray-100">
                                                ...
                                                <h4 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{{ $category->name }}</h4>
                                            </div>

                                            <!-- Slots Grid -->
                                            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                                @foreach ($categorySlots as $slot)
                                                    <div class="p-4 rounded-2xl border border-gray-200/40 bg-gray-50/20 flex flex-col justify-between">
                                                        ...
                                                        <!-- Weekly Price Grid -->
                                                        <div class="grid grid-cols-7 gap-1 mt-3 pt-3 border-t border-gray-100">
                                                            @foreach (['Mon' => 'mon', 'Tue' => 'tue', 'Wed' => 'wed', 'Thu' => 'thu', 'Fri' => 'fri', 'Sat' => 'sat', 'Sun' => 'sun'] as $dayName => $dayKey)
                                                                @php
                                                                    $price = $slot->pivot ? $slot->pivot->$dayKey : null;
                                                                @endphp
                                                                <div class="flex flex-col items-center bg-gray-50 py-1.5 px-0.5 rounded-lg border border-gray-100/50">
                                                                    <span class="text-[8px] font-bold text-gray-500 uppercase">{{ $dayName }}</span>
                                                                    <span class="text-[10px] font-mono font-bold text-indigo-600 mt-1">
