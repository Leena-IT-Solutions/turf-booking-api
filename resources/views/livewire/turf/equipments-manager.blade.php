...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function saveEquipment()
    {
        ...
        if ($this->isEditing) {
            ...
            session()->flash('status', 'Equipment added successfully.');
        }
    ...
    <div class="sm:px-6 lg:px-8 space-y-6">
        ...
        @if (!$turf)
            ...
            <div class="bg-white dark:bg-gray-800 p-12 rounded-3xl border border-gray-100 dark:border-gray-700/50 shadow-sm text-center">
                ...
                <p class="text-sm text-gray-400 mt-2 max-w-sm mx-auto leading-relaxed">{{ __('Please add a Location and Turf first, or choose one from the selector in the top bar to manage equipments.') }}</p>
            </div>
        @else
            <!-- Header Section -->
                                    ...
                                    <tr class="hover:bg-gray-50/30 dark:hover:bg-gray-800/10 transition">
                                        ...
                                        <td class="px-6 py-4.5 whitespace-nowrap text-right text-xs font-medium pr-8">
                                            <div class="inline-flex items-center gap-2">
                                                <button type="button" wire:click="openEditModal({{ $equipmentItem->id }})" class="p-2 text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400 transition cursor-pointer" title="{{ __('Edit') }}">
                                                    <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                    </svg>
                                                </button>
                                                <button type="button" wire:click="confirmDelete({{ $equipmentItem->id }})" class="p-2 text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400 transition cursor-pointer" title="{{ __('Delete') }}">
                                                    <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <form wire:submit="saveEquipment" class="space-y-4 mt-4">
                            ...
                            <div class="flex items-center justify-between p-3 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-250 dark:border-gray-700">
                                <div class="flex flex-col">
                                    <span class="text-xs font-bold text-gray-800 dark:text-gray-200">{{ __('Equipment Active Status') }}</span>
                                    <span class="text-[10px] text-gray-400 mt-0.5">{{ __('Active equipment options are visible to end users during booking.') }}</span>
                                </div>
