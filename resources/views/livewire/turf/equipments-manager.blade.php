            ...
            <form wire:submit="saveEquipments" class="space-y-6">
                ...
                @if ($allEquipments->isEmpty())
                    ...
                    <div class="bg-white dark:bg-gray-800 p-16 rounded-3xl border border-gray-100 dark:border-gray-700/50 shadow-sm text-center">
                        ...
                        <h3 class="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">{{ __('No Global Equipments Configured') }}</h3>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-2 max-w-xs mx-auto leading-relaxed">{{ __('Please ask the SaaS Administrator to add equipments to the global repository first.') }}</p>
                    </div>
                @else
                    <!-- Checkbox Grid -->
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700/50 shadow-sm">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            @foreach ($allEquipments as $equipmentItem)
                                <label class="relative flex items-center p-4 rounded-2xl border transition duration-150 cursor-pointer {{ in_array((string)$equipmentItem->id, $selectedEquipmentIds) ? 'bg-indigo-50/40 border-indigo-600 dark:bg-indigo-950/20 dark:border-indigo-500 ring-2 ring-indigo-600/10 dark:ring-indigo-500/10' : 'bg-transparent border-gray-100 hover:bg-gray-50/50 dark:border-gray-700 dark:hover:bg-gray-800/30' }}">
                                    <div class="flex items-center h-5">
                                        <input type="checkbox" wire:model="selectedEquipmentIds" value="{{ $equipmentItem->id }}" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500/20 dark:border-gray-750 dark:bg-gray-900 dark:checked:bg-indigo-600 transition">
                                    </div>
                                    <div class="ms-3 flex items-center gap-3">
                                        ...
                                        <span class="font-semibold text-sm text-gray-900 dark:text-gray-100">{{ $equipmentItem->name }}</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>
