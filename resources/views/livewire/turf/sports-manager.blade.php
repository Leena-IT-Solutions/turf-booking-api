        ...
        @else
            ...
            <form wire:submit="saveSports" class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">{{ __('Sports for') }} <span class="text-indigo-600">{{ $turf->name }}</span></h2>
                        <p class="text-xs text-gray-400 mt-1">{{ __('Select what sports categories or match formats are supported at this turf court.') }}</p>
                    </div>
                ...
                @if ($allSports->isEmpty())
                    ...
                    <div class="bg-white p-16 rounded-3xl border border-gray-100 shadow-sm text-center">
                        ...
                        <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wider">{{ __('No Global Sports Configured') }}</h3>
                        <p class="text-xs text-gray-400 mt-2 max-w-xs mx-auto leading-relaxed">{{ __('Please ask the SaaS Administrator to add sports to the global repository first.') }}</p>
                    </div>
                @else
                    <!-- Checkbox Grid -->
                    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            @foreach ($allSports as $sportItem)
                                <label class="relative flex items-center p-4 rounded-2xl border transition duration-150 cursor-pointer {{ in_array((string)$sportItem->id, $selectedSportIds) ? 'bg-indigo-50/40 border-indigo-600 ring-2 ring-indigo-600/10 ' : 'bg-transparent border-gray-100 hover:bg-gray-50/50 ' }}">
                                    <div class="flex items-center h-5">
                                        <input type="checkbox" wire:model="selectedSportIds" value="{{ $sportItem->id }}" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500/20 transition">
                                    </div>
                                    <div class="ms-3 flex items-center gap-3">
                                        ...
                                        <span class="font-semibold text-sm text-gray-900">{{ $sportItem->name }}</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>
