...
<div class="w-full space-y-6" x-data="{ uploadModal: @entangle('showUploadModal'), deleteConfirm: @entangle('showDeleteConfirm') }">
        ...
        @if (!$turf)
            ...
            <div class="bg-white p-12 rounded-3xl border border-gray-100 shadow-sm text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900">{{ __('No Turf Selected') }}</h3>
        ...
        @else
            ...
            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">{{ __('Photos for') }} <span class="text-indigo-600">{{ $turf->name }}</span></h2>
                    <p class="text-xs text-gray-400 mt-1">{{ __('Manage your turf field image gallery, activate visual showcases, or upload new high-resolution court photos.') }}</p>
                </div>
            ...
            @if ($photos->isEmpty())
                ...
                <div class="bg-white p-16 rounded-3xl border border-gray-100 shadow-sm text-center">
                    ...
                    <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wider">{{ __('No Photos Uploaded') }}</h3>
                    <p class="text-xs text-gray-400 mt-2 max-w-xs mx-auto leading-relaxed">{{ __('Start uploading gallery photos to show customers the look and feel of this turf.') }}</p>
                </div>
            @else
                <!-- Photos Grid -->
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($photos as $photo)
                        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col group relative">
                            <!-- Image Wrapper -->
                            <div class="aspect-video w-full bg-gray-100 relative overflow-hidden">
                                ...
                                <div class="absolute inset-0 bg-gray-950/30 opacity-0 group-hover:opacity-100 transition duration-300 flex items-center justify-center gap-3">
                                    <!-- Toggle Active -->
                                    <button type="button" wire:click="toggleActive({{ $photo->id }})" class="p-2.5 rounded-xl bg-white/90 hover:bg-white text-gray-700 shadow-lg cursor-pointer transition" title="{{ $photo->is_active ? __('Deactivate') : __('Activate') }}">
                                        @if ($photo->is_active)
                                    ...
                                    <!-- Delete Button -->
                                    <button type="button" wire:click="confirmDelete({{ $photo->id }})" class="p-2.5 rounded-xl bg-white/90 hover:bg-white text-red-500 hover:text-red-600 shadow-lg cursor-pointer transition" title="{{ __('Delete') }}">
        ...
        <div x-show="uploadModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div x-show="uploadModal" @click="uploadModal = false" class="fixed inset-0 transition-opacity bg-gray-950/40 backdrop-blur-sm"></div>

                <!-- Center element helper -->
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div x-show="uploadModal" class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-100">
                    <div class="p-6">
                        <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                            <h3 class="text-base font-bold text-gray-900">
                                {{ __('Upload Turf Photo') }}
                            </h3>
                            <button type="button" @click="uploadModal = false" class="text-gray-400 hover:text-gray-500 cursor-pointer">
                        ...
                        <form wire:submit="uploadPhoto" class="space-y-4 mt-4">
                            ...
                            <div>
                                ...
                                <div 
                                    ...
                                    :class="isDragging ? 'border-indigo-500 bg-indigo-50/40 scale-[1.01]' : '{{ $errors->has('photoFile') ? 'border-red-500 bg-red-50/10 ' : 'border-gray-300 bg-gray-50/50 ' }}'"
                                ...
                                >
                                    ...
                                    <div x-show="isUploading" class="absolute inset-0 bg-white/90 backdrop-blur-xs flex flex-col items-center justify-center rounded-2xl z-20" style="display: none;">
                                        <svg class="animate-spin h-8 w-8 text-indigo-650" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
                                        <span class="text-xs font-bold text-gray-700 mt-2" x-text="'Uploading image (' + progress + '%)...'"></span>
                                    </div>
                            ...
                            @if ($photoFile && !$errors->has('photoFile'))
                                <div class="mt-4">
                                    ...
                                    <div class="aspect-video w-full rounded-2xl overflow-hidden border border-gray-100 bg-gray-50">
                            ...
                            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                                <button type="button" @click="uploadModal = false" class="px-4 py-2 text-xs font-bold text-gray-700 border border-gray-250 rounded-xl hover:bg-gray-50 transition cursor-pointer">
                                    {{ __('Cancel') }}
                                </button>
